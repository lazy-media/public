---
description: >-
  A chat app that puts you first. Made in Sweden. Fluxer is a free and open
  source instant messaging and VoIP chat app built for friends, groups, and
  communities.
---
...
You should now have `Caddyfile`, `docker-compose.yml`, `livekit.yaml`, and `.env`.
{% endstep %}
...
{% tabs %}
{% tab title="Direct public server" %}
...
Open the web app at `https://chat.example.com`.

Register the first account. On a self-hosted instance, the first accepted registration receives wildcard admin access. Use that account for the initial admin login at `https://chat.example.com/admin`.
